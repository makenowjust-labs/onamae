...
private final case class ProductOrbit[A, B](orbitA: Nominal[A]#Orbit, orbitB: Nominal[B]#Orbit, keys: Seq[Key])

/** KeyGen is a generator function of keys. */
private type KeyGen = (Int, Int) => Seq[Seq[Key]]

/** Keys utilities. */
private object KeyGen:
  import Key.{Left, Both, Right}

  val productMemo: mutable.Map[(Int, Int), Seq[Seq[Key]]] = mutable.Map.empty
  val sepProductMemo: mutable.Map[(Int, Int), Seq[Seq[Key]]] = mutable.Map.empty
  val leftProductMemo: mutable.Map[(Int, Int), Seq[Seq[Key]]] = mutable.Map.empty
  val rightProductMemo: mutable.Map[(Int, Int), Seq[Seq[Key]]] = mutable.Map.empty

  /** Returns a sequence of keys for a product of `n` and `m` dimension orbits. */
  def product(n: Int, m: Int): Seq[Seq[Key]] =
    productMemo.getOrElseUpdate(
      (n, m),
      (n, m) match
        ...
        case (n, m) =>
          product(n - 1, m).map(Left +: _) ++ product(n - 1, m - 1).map(Both +: _) ++ product(n, m - 1).map(Right +: _)
    )
  ...
  /** Like `Key.productKeys`, but these keys are separated. That is, a product generated by these keys has no
    ...
    */
  def sepProduct(n: Int, m: Int): Seq[Seq[Key]] =
    sepProductMemo.getOrElseUpdate(
      (n, m),
      (n, m) match
        case (0, 0) => Seq(Seq.empty)
        case (n, 0) => Seq(Seq.tabulate(n)(_ => Left))
        ...
        case (n, m) =>
          sepProduct(n - 1, m).map(Left +: _) ++ sepProduct(n, m - 1).map(Right +: _)
    )

  /** Like `Key.productKeys`, but it is for left products. */
  def leftProduct(n: Int, m: Int): Seq[Seq[Key]] =
    leftProductMemo.getOrElseUpdate(
      (n, m),
      (n, m) match
        case (0, 0)          => Seq(Seq.empty)
        case (n, 0)          => Seq(Seq.tabulate(n)(_ => Left))
        ...
        case (n, m) =>
          leftProduct(n - 1, m).map(Left +: _) ++ leftProduct(n - 1, m - 1).map(Both +: _)
    )

  /** Like `Key.productKeys`, but it is for right products. */
  def rightProduct(n: Int, m: Int): Seq[Seq[Key]] =
    rightProductMemo.getOrElseUpdate(
      (n, m),
      (n, m) match
        case (0, 0)          => Seq(Seq.empty)
        case (0, m)          => Seq(Seq.tabulate(m)(_ => Right))
        ...
        case (n, m) if n > m => Seq.empty
        case (n, m) =>
          rightProduct(n - 1, m - 1).map(Both +: _) ++ rightProduct(n, m - 1).map(Right +: _)
    )
...
object Nominal:
  ...
  /** Derives a trivial `Nominal` instance.
    ...
    * ```
    */
  inline def derivedTrivially[A]: Nominal[A] =
    trivialNominalInstanceCache.asInstanceOf[Nominal[A]]

  private val trivialNominalInstanceCache: Nominal[Any] = new TrivialNominalInstance

  private final class TrivialNominalInstance[A] extends Nominal[A]:
    type Orbit = A
  ...
  private inline def derivedProduct[A](mirror: Mirror.ProductOf[A]): Nominal[A] =
    ...
    newProductNominalInstance(mirror, instance)

  // This method is necessary to make `ProductNominalInstance` private because it is impossible to refer
  // a private class in `inline` methods.
  private def newProductNominalInstance[A](mirror: Mirror.ProductOf[A], instance: Nominal[Tuple]): Nominal[A] =
    new ProductNominalInstance(mirror, instance)

  private final class ProductNominalInstance[A](val mirror: Mirror.ProductOf[A], val instance: Nominal[Tuple])
      extends Nominal[A]:
  ...
  private inline def derivedSum[A](mirror: Mirror.SumOf[A]): Nominal[A] =
    ...
    newSumNominalInstance(mirror, instances)
  ...
  // This method is necessary to make `SumNominalInstance` private because it is impossible to refer
  // a private class in `inline` methods.
  private def newSumNominalInstance[A](
      mirror: Mirror.SumOf[A],
      instances: Tuple.Map[mirror.MirroredElemTypes, Nominal]
  ): Nominal[A] = new SumNominalInstance(mirror, instances)

  private final class SumNominalInstance[A](
